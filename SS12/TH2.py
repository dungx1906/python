saving_accounts = [
    {
        "account_id": "STK001",
        "customer_name": "Nguyễn Văn An",
        "balance": 50000000,
        "term_months": 6,
        "interest_rate": 6.5,
        "status": "active"
    },
    {
        "account_id": "STK002",
        "customer_name": "Trần Thị Bình",
        "balance": 120000000,
        "term_months": 12,
        "interest_rate": 7.2,
        "status": "active"
    }
]

while True:

    print("\n===== HỆ THỐNG QUẢN LÝ TÀI KHOẢN TIẾT KIỆM TECHBANK =====")
    print("1. Xem danh sách sổ tiết kiệm")
    print("2. Mở sổ tiết kiệm mới")
    print("3. Cập nhật thông tin sổ tiết kiệm")
    print("4. Tất toán sổ tiết kiệm")
    print("5. Tính lãi dự kiến")
    print("6. Kiểm tra rút trước hạn")
    print("7. Thoát")

    choice = input("Nhập lựa chọn: ")

    if not choice.isdigit():
        print("Lựa chọn không hợp lệ, vui lòng nhập lại")
        continue

    choice = int(choice)

    if choice == 1:

        if len(saving_accounts) == 0:
            print("Danh sách sổ tiết kiệm hiện đang trống")
        else:
            print("\nDanh sách sổ tiết kiệm:")

            stt = 1

            for account in saving_accounts:
                print(
                    str(stt) + ". Mã sổ:",
                    account["account_id"],
                    "| Khách hàng:",
                    account["customer_name"],
                    "| Số tiền gửi:",
                    account["balance"],
                    "| Kỳ hạn:",
                    str(account["term_months"]) + " tháng",
                    "| Lãi suất:",
                    str(account["interest_rate"]) + "%/năm",
                    "| Trạng thái:",
                    account["status"]
                )

                stt += 1

    elif choice == 2:

        account_id = input("Nhập mã sổ tiết kiệm: ").strip().upper()
        customer_name = input("Nhập tên khách hàng: ").strip()

        balance = input("Nhập số tiền gửi: ")
        term_months = input("Nhập kỳ hạn gửi theo tháng: ")
        interest_rate = input("Nhập lãi suất năm: ")

        if customer_name == "":
            print("Tên khách hàng không được để trống")
            continue

        found = False

        for account in saving_accounts:
            if account["account_id"] == account_id:
                found = True

        if found:
            print("Mã sổ tiết kiệm đã tồn tại!")
            continue

        if not balance.isdigit() or not term_months.isdigit():
            print("Số tiền gửi hoặc kỳ hạn không hợp lệ")
            continue

        if float(balance) <= 0 or int(term_months) <= 0:
            print("Số tiền gửi hoặc kỳ hạn không hợp lệ")
            continue

        if not interest_rate.replace(".", "", 1).isdigit():
            print("Lãi suất không hợp lệ!")
            continue

        if float(interest_rate) <= 0:
            print("Lãi suất không hợp lệ!")
            continue

        saving_accounts.append({
            "account_id": account_id,
            "customer_name": customer_name,
            "balance": int(balance),
            "term_months": int(term_months),
            "interest_rate": float(interest_rate),
            "status": "active"
        })

        print("Mở sổ tiết kiệm thành công!")

    elif choice == 3:

        account_id = input("Nhập mã sổ cần cập nhật: ").strip().upper()

        found = False

        for account in saving_accounts:

            if account["account_id"] == account_id:

                found = True

                if account["status"] == "closed":
                    print("Không thể cập nhật sổ tiết kiệm đã tất toán!")
                    break

                customer_name = input("Tên khách hàng mới: ").strip()

                if customer_name == "":
                    print("Tên khách hàng không được để trống")
                    break

                balance = input("Số tiền gửi mới: ")
                term_months = input("Kỳ hạn mới: ")
                interest_rate = input("Lãi suất mới: ")

                if not balance.isdigit() or not term_months.isdigit():
                    print("Số tiền gửi hoặc kỳ hạn không hợp lệ")
                    break

                if float(balance) <= 0 or int(term_months) <= 0:
                    print("Số tiền gửi hoặc kỳ hạn không hợp lệ")
                    break

                if not interest_rate.replace(".", "", 1).isdigit():
                    print("Lãi suất không hợp lệ!")
                    break

                if float(interest_rate) <= 0:
                    print("Lãi suất không hợp lệ!")
                    break

                account["customer_name"] = customer_name
                account["balance"] = int(balance)
                account["term_months"] = int(term_months)
                account["interest_rate"] = float(interest_rate)

                print("Cập nhật thành công!")
                break

        if not found:
            print("Không tìm thấy mã sổ tiết kiệm!")

    elif choice == 4:

        account_id = input(
            "Nhập mã sổ tiết kiệm cần tất toán/xóa: "
        ).strip().upper()

        found = False

        for account in saving_accounts:

            if account["account_id"] == account_id:

                found = True
                account["status"] = "closed"

                print("Tất toán thành công!")
                break

        if not found:
            print("Không tìm thấy mã sổ tiết kiệm!")

    elif choice == 5:

        account_id = input(
            "Nhập mã sổ tiết kiệm cần tính lãi: "
        ).strip().upper()

        found = False

        for account in saving_accounts:

            if account["account_id"] == account_id:

                found = True

                if account["status"] == "closed":
                    print("Không thể thao tác với sổ tiết kiệm đã tất toán")
                    break

                interest = (
                    account["balance"]
                    * account["interest_rate"]
                    / 100
                    * account["term_months"]
                    / 12
                )

                total = account["balance"] + interest

                print("Tiền lãi dự kiến:", interest)
                print("Tổng tiền nhận:", total)

                break

        if not found:
            print("Không tìm thấy mã sổ tiết kiệm!")

    elif choice == 6:

        account_id = input(
            "Nhập mã sổ tiết kiệm cần kiểm tra: "
        ).strip().upper()

        months_sent = input("Nhập số tháng thực gửi: ")

        if not months_sent.isdigit():
            print("Số tháng thực gửi không hợp lệ!")
            continue

        months_sent = int(months_sent)

        if months_sent <= 0:
            print("Số tháng thực gửi không hợp lệ!")
            continue

        found = False

        for account in saving_accounts:

            if account["account_id"] == account_id:

                found = True

                if account["status"] == "closed":
                    print("Không thể thao tác với sổ tiết kiệm đã tất toán")
                    break

                if months_sent < account["term_months"]:
                    rate = 0.5
                else:
                    rate = account["interest_rate"]

                interest = (
                    account["balance"]
                    * rate
                    / 100
                    * months_sent
                    / 12
                )

                total = account["balance"] + interest

                print("Lãi suất áp dụng:", rate, "%")
                print("Tiền lãi thực nhận:", interest)
                print("Tổng tiền thực nhận:", total)

                break

        if not found:
            print("Không tìm thấy mã sổ tiết kiệm!")

    elif choice == 7:
        print("Thoát chương trình!")
        break

    else:
        print("Lựa chọn không hợp lệ, vui lòng nhập lại")